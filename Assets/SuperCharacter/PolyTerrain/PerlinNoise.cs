...
namespace PerlinNoise
{
    class PerlinNoise
    {
        ...
        {
            float[][] whitenoise = GenerateWhiteNoise(width * 10, height * 10);
            float[][] smoothnoise = GenerateSmoothNoise(whitenoise, octave);
            noise = GeneratePerlinNoise(smoothnoise, octave);
        }
        ...
        public float Get(float x,float y)
        {
            try
            {
                return noise[(int)x*10][(int)y*10];
            }
            catch
            {
                return 0;
