...
public class SuperCharacterController : BaseBehaviour
{
    ...
    public List<Collider> triggerData { get; private set; }
    ...
    void Awake()
    {
        ...
        ronateQuaternion = Quaternion.AngleAxis(0, up);
    }
    ...
    private Quaternion ronateQuaternion;

    public Quaternion GetRonate()
    {
        return ronateQuaternion;
    }
    public Vector2 GetHorizontal()
    {
        Vector3 v3 = Math3d.ProjectVectorOnPlane(up, moveDirection);
    ...
    /// <summary>
    /// 朝一个方向按一定速度移动（以一定加速度加速）
    /// </summary>
    /// <param name="direction">二维移动方向的向量</param>
    /// /// <param name="speed">最高移动速度</param>
    /// /// <param name="WalkAcceleration">加速度</param>
    ...
    public void Ronate(Quaternion target,float maxDelta)
    {
        ronateQuaternion = Quaternion.RotateTowards(ronateQuaternion, target, maxDelta);
    }

    public Vector3 GetRight()
    {
        return Vector3.Cross(up, lookDirection);
    }
    ...
    {
        return currentGround.IsGrounded(false, 0.01f);
    }

    public bool MaintainingGround()
    {
        return currentGround.IsGrounded(true, 0.5f);
    }
    ...
    void SingleUpdate()
    {
        ...
        lookDirection = ronateQuaternion * Vector3.forward;
        AnimatedMesh.rotation = Quaternion.LookRotation(lookDirection, up);
